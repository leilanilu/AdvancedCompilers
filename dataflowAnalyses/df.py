...
def reaching_transfer(block, in_defs):
    #Transfer def: OUT[B] = gen[B] U (IN[B] - KILL[B])
    gen_set = gen(block)
    kill_set = gen_set  
    return gen_set.union(in_defs - kill_set)

def avail_transfer(block, in_exprs):
    #Transfer def: OUT[B] = e_gen[B] U (IN[B] - e_kill[B])
    e_gen = set()
    e_kill = set()
    ...
    for instr in block:
        dest = instr.get("dest")
        if dest:
            #add to kill and remove from gen any expression w/variable
            e_kill |= {expr for expr in in_exprs if dest in expr}
            e_gen -= {expr for expr in in_exprs if dest in expr}

            #Check if it's an exxpression + build tuple
            if "op" in instr and "args" in instr and len(instr["args"])==2:
                op = instr["op"]
                x, y = instr["args"]
    ...
    return e_gen.union(in_exprs-e_kill)
...
ANALYSES = {
    ...
    "avail": Analysis(
        True,
        init=set(),
        merge=lambda sets: set.intersection(*sets) if sets else set(),
        transfer=avail_transfer,
    ),
}
